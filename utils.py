...
def _get_weight_path(content_type, input_repr):
    return os.path.join(core.ASSETS_DIR, 'weights', f'{content_type}-{input_repr}')
...
def _check_content_type(content_type):
    ...
    assert content_type in core.CONTENT_TYPES, f'content_type must be one of {core.CONTENT_TYPES}'

def _check_input_repr(input_repr):
        # check input representation
    assert isinstance(input_repr, str)
    assert input_repr in core.INPUT_REPRESENTATIONS,  f'input_repr must be one of {core.INPUT_REPRESENTATIONS}'
